...
class InvoiceReportMixin(DominateReport):
    ...
    def show_document_info(cls, record):
        ...
        with container:
            ...
            h2('TEST STATUS UPDATE', cls='document')
            h2('%s: %s' % (
                cls.label(record.raw.__name__, 'state'),
                record.render.state), cls='document')    
        return container
    ...
    def body(cls, action, data, records):
        ...
        with container:
            ...
            #         font-size: 140px;
            #         font-weight: bold;
            #         color: rgba(200, 200, 200, 0.25); /* Light gray with 25% opacity */
            #         z-index: -9999;
            #         pointer-events: none;
            #         white-space: nowrap;
            #     }
            # """))
            # # This dynamically prints the status (e.g., "DRAFT") as the watermark
            # div(record.render.state.upper(), cls='watermark')
