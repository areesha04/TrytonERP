...
def generate_html(data):
    ...
    lines = PurchaseLine.search(domain, order=[('purchase.party', 'ASC'), ('purchase.purchase_date', 'ASC')])
    ...
    with doc:
        ...
        tags.h2("SUPPLIER-WISE PURCHASE REPORT", style="margin: 0 0 20px 0; font-size: 20px; color: #e74c3c; border-bottom: 2px solid #e74c3c; padding-bottom: 5px;")
        ...
        with tags.table(style="width: 100%; border-collapse: collapse; margin-bottom: 25px;"):
            with tags.thead():
                with tags.tr(style="background-color: #2c3e50; color: #ffffff; text-align: left;"):
                    tags.th("Supplier", style="padding: 8px;")
                    ...
                    tags.th("Total Amount", style="padding: 8px; text-align: right;")
            with tags.tbody():
                for index, line in enumerate(lines):
                    ...
                    with tags.tr(style=f"background-color: {bg_color}; border-bottom: 1px solid #ecf0f1;"):
                        tags.td(line.purchase.party.name if line.purchase and line.purchase.party else "", style="padding: 8px; font-weight: bold; color: #2c3e50;")
                        tags.td(str(line.purchase.purchase_date) if line.purchase else "", style="padding: 8px;")
                        tags.td(prod.name if prod else "", style="padding: 8px;")
