...
class VendorAdvanceDepositReport(HTMLReport):
    ...
    def get_html(cls, records, data):
        ...
        # ==========================================
        # SOURCE 1: INVOICE LINES (Old Logic)
        # ==========================================
        inv_domain = [
            ('invoice.type', '=', 'in'),
        ...
        if raw_start:
            inv_domain.append(('invoice.invoice_date', '>=', raw_start))
        if raw_end:
            inv_domain.append(('invoice.invoice_date', '<=', raw_end))
        if supplier_id:
            inv_domain.append(('invoice.party', '=', supplier_id))
            
        deposit_inv_lines = InvoiceLine.search(inv_domain)

        # ==========================================
        # SOURCE 2: MOVE LINES (New Logic)
        # ==========================================
        move_domain = [
            ('move.state', '=', 'posted'),
            ('account', 'in', adv_acc_ids if adv_acc_ids else [0]),
        ...
        if raw_start:
            move_domain.append(('date', '>=', raw_start))
        if raw_end:
            move_domain.append(('date', '<=', raw_end))
        if supplier_id:
            move_domain.append(('party', '=', supplier_id))
            
        deposit_move_lines_raw = MoveLine.search(move_domain)

        # ==========================================
        # UNIFY AND PREVENT DOUBLE COUNTING
        # ==========================================
        unified_records = []
        ...
        # Process Move Lines
        for ml in deposit_move_lines_raw:
            # Check if this move was generated by an invoice to prevent double counting
            origin = getattr(ml.move, 'origin', None)
            # Safely check if the origin is an account.invoice model
            if origin and getattr(origin, '__name__', '') == 'account.invoice':
                continue # Skip this line, it's already handled by Source 1
        ...
        # 4. Sort and Group Data by Supplier
        # Sort by party name, then by date
        unified_records.sort(key=lambda r: (r['party'].name if r['party'] else '', r['date'] or date.min))
        ...
        for rec in unified_records:
            ...
            if party not in grouped_lines:
                grouped_lines[party] = []
            grouped_lines[party].append(rec)
        ...
        with doc:
            ...
            with tags.body():
                with tags.div(cls="report-header"):
                    tags.h1("Supplier Advance Payments Ledger (Unified)")
                            ...
                            with tags.tbody():
                                ...
                                for row in v_lines:
                                    ...
                                    with tags.tr():
                                        ...
                                        tags.td(str(row['ref']))
                                        tags.td(row['desc'])  # Will display [Invoice] or [Journal]
